...
instalar(){
    ...
    if [ "$1" == "packer" ]; then
      ...
      avisar "==> Instalando os requerimentos dos plugins do $1"
...
iniciar(){
    if [ "$1" == "terraform" ]; then
        ...
        if [ ! -f "vps" ]; then 
            ...
            terraform apply "vps"
            echo "==> Aguardando 15 segundos para que o $1 plan "vps" seja completamente iniciado..."
            sleep 15
            terraform plan -out=vps
            terraform apply "vps"
        ...
        if [ ! -f "iso/jammy-server-cloudimg-amd64.img" ]; then
            echo "==> Iniciando o download da jammy-server-cloudimg-amd64.img..."
            wget http://cloud-images.ubuntu.com/jammy/current/jammy-server-cloudimg-amd64.img -O iso/jammy-server-cloudimg-amd64.img
            echo "==> Customizar a jammy-server-cloudimg-amd64.img com a --root-password password:neoricalex ..."
        ...
        sshpass -p neoricalex ssh -o StrictHostKeyChecking=accept-new neoricalex@$ip_vps 'bash -s' < scripts/inicio.sh
